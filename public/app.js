...
async function loadData() {
  ...
  const [bootstrap, day] = await Promise.all([
    api('/api/bootstrap'),
...
function renderLineChart(rows) {
  ...
  const points = rows.map((row, index) => { const x = pad.left + (rows.length === 1 ? (width - pad.left - pad.right) / 2 : index / (rows.length - 1) * (width - pad.left - pad.right)); const y = pad.top + (max - row.valueUmolL) / spread * (height - pad.top - pad.bottom); return { x, y, row }; });
...
function renderManage() {
  const tabs = [['food', '食物'], ['recipe', '菜谱'], ['beverage', '饮品'], ['group', '分组'], ['settings', '设置与数据']];
  ...
  if (state.manageTab === 'group') content = renderGroupLibrary();
...
function pickerOptions(kind, search = '') {
  const list = kind === 'recipe' ? state.bootstrap.recipes : state.bootstrap.foods;
...
function renderPicker(kind, selectedId = '', search = '') {
  ...
  return options.length ? options.map((item) => { const id = item.versionId; return `<button type="button" class="picker-option ${id === selectedId ? 'selected' : ''}" data-action="select-picker" data-version-id="${id}" data-item-name="${escapeHtml(item.name)}"><span>${escapeHtml(item.name)}<small>${escapeHtml(item.groupName || '未分组')} · ${item.purineLow === null ? '暂无估算' : formatRange(item.purineLow, item.purineHigh)} / 100g</small></span><span>${id === selectedId ? '✓' : '›'}</span></button>`; }).join('') : '<div class="empty-state" style="padding:15px">没有匹配的条目</div>';
}
...
function openDietModal(kind, edit = null) {
  ...
  openModal(edit ? '修改饮食记录' : `记录${kind === 'recipe' ? '菜谱' : '食物'}`, '服务器确认后才会显示为已保存；历史记录保留录入时快照。', `<form data-form="diet" data-id="${edit?.id || ''}" data-kind="${kind}"><div class="form-grid"><div class="form-field full"><label>记录日期</label><input name="date" type="date" value="${edit?.date || state.currentDate}" required /></div><div class="form-field full picker"><label>${kind === 'recipe' ? '搜索菜谱（名称、别名或分组）' : '搜索食物（名称、别名或分组）'}</label><input class="picker-search" type="search" placeholder="输入关键词" value="${escapeHtml(item?.name || edit?.name || '')}" autocomplete="off" /><input name="versionId" class="picker-value" type="hidden" value="${selectedId}" required /><div class="picker-results">${renderPicker(kind, selectedId, item?.name || edit?.name || '')}</div><p class="form-hint picker-selected">${item ? `已选择：${escapeHtml(item.name)}` : '请选择一个条目'}</p></div><div class="form-field full"><label>快捷份量</label><div class="preset-row">${presets.map((preset) => `<button type="button" class="preset-button" data-action="use-preset" data-value="${preset.value}">${preset.value}g</button>`).join('')}</div><input name="quantityG" type="number" step="0.1" min="0.1" value="${edit?.quantityG || ''}" placeholder="也可以输入自定义克数" required /></div></div><div class="preview-card"><small>保存前预览</small><strong class="diet-preview">请选择条目和克数</strong><p>估算范围来自当前参考版本；保存后历史记录不会随参考库静默改写。</p></div><div class="modal-actions"><button type="button" class="button button-secondary" data-action="close-modal">取消</button><button type="submit" class="button button-primary">${edit ? '保存修改' : '保存记录'}</button></div></form>`, { type: 'diet', kind });
  updateDietPreview();
}
...
function openBeverageModal(edit = null) {
  ...
  openModal(edit ? '修改饮品记录' : '记录饮品', '饮品只统计容量，不自动赋予降尿酸值。', `<form data-form="beverage" data-id="${edit?.id || ''}"><div class="form-grid"><div class="form-field full"><label>记录日期</label><input name="date" type="date" value="${edit?.date || state.currentDate}" required /></div><div class="form-field full"><label>饮品</label><select name="beverageId" required>${state.bootstrap.beverages.map((item) => `<option value="${item.id}" ${item.id === beverage?.id ? 'selected' : ''}>${escapeHtml(item.name)}${item.system ? ' · 系统预置' : ''}</option>`).join('')}</select></div><div class="form-field"><label>每份容量（mL）</label><input name="amountMl" type="number" min="0.1" step="0.1" value="${edit?.quantity > 1 ? edit.amountMl / edit.quantity : edit?.amountMl || ''}" required /></div><div class="form-field"><label>数量</label><input name="quantity" type="number" min="1" step="1" value="${edit?.quantity || 1}" required /></div><div class="form-field full"><label>快捷容量</label><div class="preset-row">${presets.map((preset) => `<button type="button" class="preset-button" data-action="use-preset" data-value="${preset.value}">${preset.value}mL</button>`).join('')}</div></div></div><div class="preview-card"><small>规范化总量</small><strong class="beverage-preview">—</strong><p>例如 500mL × 2 会保存为 1000mL。</p></div><div class="modal-actions"><button type="button" class="button button-secondary" data-action="close-modal">取消</button><button type="submit" class="button button-primary">${edit ? '保存修改' : '保存饮品'}</button></div></form>`, { type: 'beverage' });
...
function addIngredientRow(item = null) {
  ...
  row.innerHTML = `<select name="foodVersionId" required><option value="">选择食物版本</option>${state.bootstrap.foods.map((food) => `<option value="${food.versionId}" ${food.versionId === item?.foodVersionId ? 'selected' : ''}>${escapeHtml(food.name)} · ${food.purineLow === null ? '未知' : formatRange(food.purineLow, food.purineHigh)}</option>`).join('')}</select><input name="grams" type="number" min="0.1" step="0.1" value="${item?.grams || ''}" placeholder="克数" required /><button class="icon-button" type="button" data-action="remove-ingredient" aria-label="移除配料">×</button>`;
...
function openGroupModal(kind = 'food', edit = null) {
  openModal(edit ? '编辑分组' : '新增分组', '分组只用于查找和筛选，不替代完整饮食解释。', `<form data-form="group" data-kind="${kind}" data-id="${edit?.id || ''}"><div class="form-field"><label>分组类型</label><select name="kind" ${edit ? 'disabled' : ''}><option value="food" ${kind === 'food' ? 'selected' : ''}>食物</option><option value="recipe" ${kind === 'recipe' ? 'selected' : ''}>菜谱</option><option value="beverage" ${kind === 'beverage' ? 'selected' : ''}>饮品</option></select></div><div class="form-field" style="margin-top:12px"><label>名称</label><input name="name" value="${escapeHtml(edit?.name || '')}" required /></div><div class="modal-actions"><button type="button" class="button button-secondary" data-action="close-modal">取消</button><button type="submit" class="button button-primary">保存分组</button></div></form>`, { type: 'group' });
}
