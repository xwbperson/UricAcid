...
function renderGroupLibrary() {
  ...
  return `<section class="panel"><div class="panel-header"><div><h4>分组</h4><p>删除分组只会把内容移到“未分组”，不会级联删除。</p></div><button class="button button-primary" data-action="open-group-form">新增分组</button></div><div class="library-list">${groups.map((group) => `<div class="library-item"><div><strong>${escapeHtml(group.name)}</strong><small>${escapeHtml(group.typeName)}${group.system ? ' · 系统预置' : ''}</small></div><div>${group.system ? '' : `<button class="text-button" data-action="open-group-form" data-kind="${escapeHtml(group.kind)}" data-id="${escapeHtml(group.id)}">编辑</button><button class="text-button" data-action="delete-group" data-kind="${escapeHtml(group.kind)}" data-id="${escapeHtml(group.id)}">删除</button>`}</div></div>`).join('')}</div></section>`;
}
