...
function renderManage() {
  const tabs = [['food', '食物'], ['recipe', '菜谱'], ['beverage', '饮品'], ['medicine', '药品'], ['settings', '设置与数据']];
  ...
  if (state.manageTab === 'medicine') content = renderMedicineLibrary();
...
function renderMedicineLibrary() {
  const allMedicines = state.bootstrap.medicines || [];
  const medicines = state.medicineKindFilter ? allMedicines.filter((medicine) => medicine.kind === state.medicineKindFilter) : allMedicines;
  const count = state.bootstrap.counts?.medicines ?? allMedicines.length;
  return `<div class="manage-layout manage-library-layout"><section class="panel"><div class="panel-header"><div><h4>常用药品</h4><p>只记录你实际使用过的口服药和外用药；这里不提供医学推荐。</p></div><button class="button button-primary" data-action="open-medicine-form">新增药品</button></div><div class="library-filter"><label for="medicine-kind-filter">按类型筛选药品</label><select id="medicine-kind-filter" data-medicine-kind-filter><option value="" ${!state.medicineKindFilter ? 'selected' : ''}>全部药品（${count}）</option><option value="oral_medication" ${state.medicineKindFilter === 'oral_medication' ? 'selected' : ''}>口服药</option><option value="topical_medication" ${state.medicineKindFilter === 'topical_medication' ? 'selected' : ''}>外用药</option></select></div><div class="library-list" data-library-list="medicine">${medicines.length ? medicines.map((medicine) => `<div class="library-item"><div><strong>${escapeHtml(medicine.name)}</strong><small>${escapeHtml(medicine.kindLabel)}${medicine.aliases ? ` · 别名：${escapeHtml(medicine.aliases)}` : ''}${medicine.notes ? ` · ${escapeHtml(medicine.notes)}` : ''}</small></div><div><span class="status-label">${escapeHtml(medicine.kindLabel)}</span><button class="text-button" data-action="open-medicine-form" data-id="${escapeHtml(medicine.id)}">编辑</button><button class="text-button" data-action="archive-medicine" data-id="${escapeHtml(medicine.id)}">归档</button></div></div>`).join('') : `<div class="empty-state"><strong>${allMedicines.length ? '当前类型还没有药品' : '还没有常用药品'}</strong><span>添加后，治疗记录中的口服药或外用药可以直接选择。</span></div>`}</div></section><section class="panel"><div class="panel-header"><div><h4>记录时直接选择</h4><p>药品资料和每次治疗记录保持分开。</p></div><span class="mono">SHORTCUTS</span></div><div class="library-list"><div class="group-unassigned"><div><strong>口服药</strong><span>在“口服药”治疗记录中，只显示口服药资料。</span></div></div><div class="group-unassigned"><div><strong>外用药</strong><span>在“外用药”治疗记录中，只显示外用药资料。</span></div></div></div><div class="info-strip">药品只作为快捷资料；剂量、频次、起止日期仍在每条治疗记录中单独填写。归档后不会出现在新记录选择器中，已有记录仍保留原名称。</div></section></div>`;
}
...
function treatmentMedicineOptions(kind, selectedId = '') {
  const medicines = (state.bootstrap?.medicines || []).filter((medicine) => medicine.kind === kind);
  return `<option value="">手动填写其他药品</option>${medicines.map((medicine) => `<option value="${escapeHtml(medicine.id)}" ${medicine.id === selectedId ? 'selected' : ''}>${escapeHtml(medicine.name)}${medicine.aliases ? ` · ${escapeHtml(medicine.aliases)}` : ''}</option>`).join('')}`;
}

function syncTreatmentMedicineOptions(form) {
  const select = $('[name="medicineId"]', form);
  if (!select) return;
  const kind = $('[name="eventType"]', form)?.value || '';
  const selectedId = select.value;
  select.innerHTML = treatmentMedicineOptions(kind, selectedId);
...
function openMedicineModal(editId = null) {
  const medicine = (state.bootstrap.medicines || []).find((item) => item.id === editId);
  const kind = medicine?.kind || 'oral_medication';
  openModal(editId ? '编辑常用药品' : '新增常用药品', '只保存你自己的常用药品资料；这里不提供用药推荐。', `<form data-form="medicine" data-id="${escapeHtml(editId || '')}"><div class="form-grid"><div class="form-field full"><label>药品类型</label><select name="kind" ${editId ? 'disabled' : ''}><option value="oral_medication" ${kind === 'oral_medication' ? 'selected' : ''}>口服药</option><option value="topical_medication" ${kind === 'topical_medication' ? 'selected' : ''}>外用药</option></select>${editId ? `<input type="hidden" name="kind" value="${escapeHtml(kind)}" />` : ''}</div><div class="form-field full"><label>药品名称</label><input name="name" value="${escapeHtml(medicine?.name || '')}" placeholder="按包装或医嘱填写" required /></div><div class="form-field full"><label>别名（可选）</label><input name="aliases" value="${escapeHtml(medicine?.aliases || '')}" placeholder="多个别名用逗号分隔" /></div><div class="form-field full"><label>备注（可选）</label><textarea name="notes" placeholder="只记录便于识别的事实，例如剂型或常用品牌">${escapeHtml(medicine?.notes || '')}</textarea></div></div><p class="form-hint">剂量、频次和起止日期不放在药品资料中；请在每条治疗记录里按实际情况填写。</p><div class="modal-actions"><button type="button" class="button button-secondary" data-action="close-modal">取消</button><button type="submit" class="button button-primary">保存${editId ? '修改' : '药品'}</button></div></form>`, { type: 'medicine' });
}
...
document.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (action === 'open-medicine-management') { state.manageTab = 'medicine'; setRoute('manage'); return; }
    ...
    if (action === 'open-medicine-form') { openMedicineModal(button.dataset.id); return; }
    if (action === 'archive-medicine' && confirm('归档这条常用药品？已有治疗记录会保留，之后的新记录不再显示它。')) { await api(`/api/medicines/${button.dataset.id}`, { method: 'DELETE' }); showToast('常用药品已归档'); await loadData(); renderCurrentRoute(); return; }
...
document.addEventListener('change', async (event) => {
  ...
  if (event.target.matches('[data-medicine-kind-filter]')) { state.medicineKindFilter = event.target.value; await renderCurrentRoute(); }
  ...
  if (event.target.matches('[data-form="treatment"] [name="medicineId"]')) { const form = event.target.closest('[data-form="treatment"]'); const medicine = (state.bootstrap.medicines || []).find((item) => item.id === event.target.value); if (medicine) $('[name="medicineName"]', form).value = medicine.name; else if (!event.target.value) $('[name="medicineName"]', form).value = ''; }
...
document.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    if (form.dataset.form === 'medicine') { const body = { name: data.name, aliases: data.aliases, kind: data.kind, notes: data.notes }; const url = form.dataset.id ? `/api/medicines/${form.dataset.id}` : '/api/medicines'; await api(url, { method: form.dataset.id ? 'PUT' : 'POST', body: JSON.stringify(body) }); closeModal(); showToast(form.dataset.id ? '常用药品已更新' : '常用药品已保存'); await loadData(); renderCurrentRoute(); return; }
